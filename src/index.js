...
class WarmUp {
  ...
  async afterPackageInitialize() {
    ...
    await this.createWarmUpFunctionArtifact(
      ...
      this.warmupOpts.xRaySdkInstalled,
      ...
      package: typeof config.package === 'object'
        ? {
          ...
          include: Array.isArray(config.package.include)
            ? [...new Set([...config.package.include, `${folderName}/**`, 'node_modules/aws-xray-sdk-core/**'])]
            : [`${folderName}/**`, 'node_modules/aws-xray-sdk-core'],
        }
  ...
  static getFunctionConfig(possibleConfig, defaultOpts) {
    ...
    return {
      ...
      tracing: (config.tracing !== undefined)
        ? config.tracing
        : defaultOpts.tracing,
    };
    /* eslint-enable no-nested-ternary */
   ...
   * @return {Promise}
   * */
  async createWarmUpFunctionArtifact(functions, region, xRaySdkInstalled, pathFile) {
    /** Log warmup start */
    this.serverless.cli.log(`WarmUp: setting ${functions.length} lambdas to be warm`);

    /** Log functions being warmed up */
    functions.forEach((func) => {
      this.serverless.cli.log(`WarmUp: ${func.name}`);
      if (func.tracing && !xRaySdkInstalled) {
        this.serverless.cli.log(`WarmUp: ${func.name} has tracing enabled but 'aws-xray-sdk-core' is not installed.`);
      }
    });

    const sdkRequire = xRaySdkInstalled
...
module.exports.warmUp = async (event, context) => {
  ...
  static addWarmUpFunctionToService(service, warmupOpts) {
    ...
    service.functions.warmUpPlugin = {
      ...
      ...(warmupOpts.tracing ? { tracing: warmupOpts.tracing } : {}),
    };
  }
