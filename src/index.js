...
class WarmUP {
  ...
  configPlugin() {
    ...
    if (typeof this.custom.warmup.es6 === "boolean") {
      this.warmup.es6 = this.custom.warmup.es6;
    }

    if (typeof this.custom.warmup.payload === "object") {
      this.warmup.payload = this.custom.warmup.payload;
    }
  }
  ...
  createWarmUpFunctionArtifact(functionNames) {
    ...
    this.serverless.cli.log(
      `WarmUP: setting ${functionNames.length} lambdas to be warm`
    );
    ...
    const fnPrefix = `aws.config.region = "${this.options.region}";
...
const params = {
  ...
  Payload: '${JSON.stringify(this.warmup.payload)}'
};
    `;
  ...
  console.log(\`Warm Up Finished with \${invokes.filter(r => !r).length} invoke errors\`);`;
    ...
    if (this.custom.warmup.es6) {
      warmUpFunction = `"use strict";
/** Generated by Serverless WarmUP Plugin at ${new Date().toISOString()} */
import aws from 'aws-sdk';

${fnPrefix}

export async function warmUp(event, context, callback) {
  ${fnBody}
}`;
    } else {
      warmUpFunction = `"use strict";
