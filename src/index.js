...
class WarmUp {
  ...
  async afterPackageInitialize() {
    ...
    await this.createWarmUpFunctionArtifact(
      ...
      { region: this.resolvedOptions.region, timeout: this.warmupOpts.timeout },
   ...
   * @return {Promise}
   * */
  async createWarmUpFunctionArtifact(functions, opts, pathFile) {
    /** Log warmup start */
    ...
    const connectTimeout = Math.min(1000, Math.round((opts.timeout * 1000) / 3));
...
const lambda = new aws.Lambda({
  ...
  httpOptions: {
    ...
    timeout: ${opts.timeout * 1000 - connectTimeout - 300}
  }
});
