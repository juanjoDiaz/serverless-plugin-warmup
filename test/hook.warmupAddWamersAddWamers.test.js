...
describe('Serverless warmup plugin warmup:warmers:addWarmers:addWarmers hook', () => {
	...
	it('Should support multiple warmers', async () => {
		const serverless = getServerlessConfig({
			service: {
				...
				functions: {
					...
					someFunc3: {
						name: 'someFunc3',
						warmup: { secondary: { enabled: false }, tertiary: { enabled: false } },
					},
				},
			},
		});
		...
		expect(plugin.serverless.service.functions.warmUpPluginDefault).toEqual(
			getExpectedFunctionConfig(),
		);
		expect(fs.mkdir).toHaveBeenCalledTimes(3);
		expect(fs.mkdir).toHaveBeenNthCalledWith(1, path.join('testPath', '.warmup', 'default'), {
			recursive: true,
		});
		expect(fs.mkdir).toHaveBeenNthCalledWith(2, path.join('testPath', '.warmup', 'secondary'), {
			recursive: true,
		});
		expect(fs.mkdir).toHaveBeenNthCalledWith(3, path.join('testPath', '.warmup', 'tertiary'), {
			recursive: true,
		});
		expect(fs.writeFile).toHaveBeenCalledTimes(3);
		expect(fs.writeFile).toHaveBeenNthCalledWith(
			1,
			path.join('testPath', '.warmup', 'default', 'index.mjs'),
			expect.anything(),
		);
		expect(fs.writeFile).toHaveBeenNthCalledWith(
			2,
			path.join('testPath', '.warmup', 'secondary', 'index.mjs'),
			expect.anything(),
		);
		expect(fs.writeFile).toHaveBeenNthCalledWith(
			3,
			path.join('testPath', '.warmup', 'tertiary', 'index.mjs'),
			expect.anything(),
		);
		...
		expect(function1Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			1,
			getExpectedLambdaCallOptions('someFunc1'),
		);
		expect(function1Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			2,
			getExpectedLambdaCallOptions('someFunc2'),
		);
		expect(function1Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			3,
			getExpectedLambdaCallOptions('someFunc3'),
		);
		...
		expect(function2Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			1,
			getExpectedLambdaCallOptions('someFunc1'),
		);
		expect(function2Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
		...
		expect(function3Tester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			1,
			getExpectedLambdaCallOptions('someFunc1'),
		);
	});
	...
	it('Should error if unknown warmers are used in a function', async () => {
		const serverless = getServerlessConfig({
			service: {
				...
				functions: {
					someFunc1: { name: 'someFunc1', warmup: { unknown: { enabled: true } } },
		...
		try {
			...
			expect(err.message).toEqual(
				'WarmUp: Invalid function-level warmup configuration (unknown) in function someFunc1. Every warmer should be declared in the custom section.',
			);
	...
	it('Should warmup all functions if globally enabled', async () => {
		...
		expect(functionTester.lambdaInstances[0]).toHaveBeenNthCalledWith(
			1,
			getExpectedLambdaCallOptions('someFunc1'),
		);
		expect(functionTester.lambdaInstances[0]).toHaveBeenNthCalledWith(
	...
	it('Should use the architecture options if present', async () => {
		...
		expect(plugin.serverless.service.functions.warmUpPluginDefault).toEqual(
			getExpectedFunctionConfig({ architecture: 'x86_64' }),
		);
	});
	...
	it('Should override provider architecture setting if set up at the warmer config', async () => {
		...
		expect(plugin.serverless.service.functions.warmUpPluginDefault).toEqual(
			getExpectedFunctionConfig({ architecture: 'arm64' }),
		);
	...
	it('Should enable X-Ray tracing if set up at the warmer config', async () => {
		...
		expect(plugin.serverless.service.functions.warmUpPluginDefault).toEqual(
			getExpectedFunctionConfig({ tracing: true }),
		);
		expect(exec).toHaveBeenCalledTimes(2);
		expect(exec).toHaveBeenNthCalledWith(
			1,
			'npm init -y',
			{ cwd: path.join('testPath', '.warmup', 'default') },
			expect.anything(),
		);
		expect(exec).toHaveBeenNthCalledWith(
			2,
	...
	it('Should overide provider tracing setting if set up at the warmer config', async () => {
		...
		expect(plugin.serverless.service.functions.warmUpPluginDefault).toEqual(
			getExpectedFunctionConfig({ tracing: false }),
		);
		expect(exec).not.toHaveBeenCalled();
	});
		...
		const serverless = getServerlessConfig({
			service: {
				...
				functions: {
					someFunc1: {
						name: 'someFunc1',
						warmup: { default: { clientContext: { othersource: 'test' } } },
					},
					someFunc2: { name: 'someFunc2' },
				},
			},
		...
		const serverless = getServerlessConfig({
			service: {
				...
				functions: {
					someFunc1: {
						name: 'someFunc1',
						warmup: { default: { payload: { othersource: 'test' } } },
					},
					someFunc2: { name: 'someFunc2' },
				},
			},
		...
		const serverless = getServerlessConfig({
			service: {
				...
				functions: {
					someFunc1: {
						name: 'someFunc1',
						warmup: { default: { payload: { test: 'value' }, payloadRaw: false } },
					},
					someFunc2: { name: 'someFunc2' },
				},
			},
	...
	describe('Other plugins integrations', () => {
		it('Should use the warmup function alias if SERVERLESS_ALIAS env variable is present', async () => {
			...
			const functionTester = new GeneratedFunctionTester(fs.writeFile.mock.calls[0][1]);
			functionTester.executeWarmupFunction({
				process: { env: { SERVERLESS_ALIAS: 'TEST_ALIAS' } },
			});
