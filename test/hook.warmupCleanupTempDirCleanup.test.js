...
jest.mock('fs', () => ({
	promises: {
		mkdir: jest.fn(),
		readdir: jest.fn((file) =>
			!file.endsWith('node_modules') && !file.endsWith('.warmup')
				? ['index.mjs', 'node_modules']
				: [],
		),
		writeFile: jest.fn(),
		rm: jest.fn(),
...
describe('Serverless warmup plugin warmup:cleanupTempDir:cleanup hook', () => {
	...
	it('Should clean the temporary folder if cleanFolder is set to true', async () => {
		...
		expect(fs.rm).toHaveBeenNthCalledWith(1, path.join('testPath', '.warmup', 'default'), {
			recursive: true,
		});
		expect(fs.rm).toHaveBeenNthCalledWith(2, path.join('testPath', '.warmup'), { recursive: true });
		expect(pluginUtils.log.error).not.toHaveBeenCalledWith(
			expect.stringMatching(/^WarmUp: Couldn't clean up temporary folder .*/),
		);
	});
	...
	it("Should not error if couldn't clean up the custom temporary folder", async () => {
		...
		expect(fs.rm).toHaveBeenCalledTimes(2);
		expect(fs.rm).toHaveBeenCalledWith(path.join('testPath', '.warmup', 'default'), {
			recursive: true,
		});
	...
	it('Should ignore cleaning the warmer temporary folders if there was nothing to clean', async () => {
		...
		expect(fs.rm).toHaveBeenNthCalledWith(2, path.join('testPath', '.warmup'), { recursive: true });
		expect(plugin.log.error).not.toHaveBeenCalledWith(
			expect.stringMatching(/^Middleware: Couldn't clean up temporary folder .*/),
		);
	});

	it("Should not error if couldn't clean up the .warmup temporary folder", async () => {
		fs.readdir.mockImplementation((dir) =>
			dir === path.join('testPath', '.warmup')
				? Promise.reject(new Error("Folder couldn't be cleaned"))
				: Promise.resolve([]),
		);
		const serverless = getServerlessConfig({
			service: {
