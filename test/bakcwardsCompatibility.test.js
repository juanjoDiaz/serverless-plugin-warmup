...
describe('Backward compatibility', () => {
	...
	describe('servicePath renamed to serviceDir', () => {
		...
		it('should fallback to servicePath if serviceDir is not defined', async () => {
			...
			expect(fs.mkdir).toHaveBeenCalledWith(path.join('testPath', '.warmup', 'default'), {
				recursive: true,
			});
		...
		it("should fallback to '' if serviceDir and servicePath are not defined", async () => {
			...
			expect(fs.mkdir).toHaveBeenCalledTimes(1);
			expect(fs.mkdir).toHaveBeenCalledWith(path.join('', '.warmup', 'default'), {
				recursive: true,
			});
			expect(fs.writeFile).toHaveBeenCalledTimes(1);
			expect(fs.writeFile).toHaveBeenCalledWith(
				path.join('', '.warmup', 'default', 'index.mjs'),
				expect.anything(),
			);
		});
	});
